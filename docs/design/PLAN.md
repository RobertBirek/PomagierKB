...
| 2026-09-07 | Izolacja baz wiedzy per użytkownik nierozstrzygnięta (MCP ma profile per klucz, panel — nic) | **Świadoma decyzja: panel NIE ogranicza baz per użytkownik.** Każdy zalogowany (także `viewer`) przeszukuje wszystkie aktywne bazy; jedyną kontrolą jest RBAC roli na trasie. Izolacja per-klucz w MCP zostaje bez zmian | Wiedza firmowa ma być dostępna wszystkim pracownikom; osobne ACL na bazach dokładałoby modelu uprawnień bez odbiorcy. Odnotowane, bo zewnętrzny audyt (2026-09-07) słusznie wskazał różnicę między MCP a panelem jako podejrzaną — to nie przeoczenie, tylko wybór. **Warunek ponownego otwarcia:** pierwsza baza z danymi kadrowymi/osobowymi | `apps/panel-api/src/services/ask.ts` (`activeNamespaces()` — wszystkie aktywne KB); `apps/mcp-server/src/tools/common.ts` (`resolveRequestedNamespaces` — profil klucza); `docs/data-governance.md` §Zakres widoczności |
| 2026-09-07 | Brak warstwy ochrony danych osobowych na wyjściu do dostawcy LLM | **Polityka PII per baza wiedzy** (`kb_registry.pii_policy`: `off`/`flag`/`mask`, domyślnie `flag`). Detekcja z sumami kontrolnymi (PESEL/NIP/REGON/IBAN/dowód) plus kontekstowe telefon i data urodzenia; maskowanie na EGRESSIE (`wrapUntrusted`), nie przy zapisie | Treść dokumentów wychodzi do dostawcy **poza EOG**; kontrola musi istnieć zanim do baz trafią akta pracownicze, bo wysyłki nie da się cofnąć. Maskowanie przy ingeście trwale niszczyłoby treść i nie chroniłoby niczego więcej | `packages/shared/src/pii/`; migracja `0061_pii_feedback_telemetry.sql`; `packages/shared/src/llm/untrusted.ts` (parametr polityki); testy `packages/shared/test/pii.test.ts` (20 przypadków, w tym negatywne na kodach katalogowych) |
| 2026-09-09 | Brak decyzji o źródłach strukturalnych (żywe bazy) i imporcie masowym | **Źródła strukturalne czytane wyłącznie narzędziem hostowym**, bez drogi kontener→tunel WireGuard; import masowy = narzędzia hostowe zasilające istniejący pipeline przez `POST /content` (JSON `{text,title,sourceUrl}`), bez nowych tras w panel-api i bez zmian w schemacie OpenSPG | `wg_guard.sh` celowo odcina kontenery od sieci biurowej (runbook `wireguard.md`); job w panel-api wymagałby wyjątku w guardzie i poświadczeń w kontenerze. Konwersja na hoście daje kontrolę podziału (limit 100 000 zn., okno analyze 12 000 zn.) i unikalny `sourceUrl` per fragment (precedencja `source_ref`) | `tools/mssql-introspect/` (tylko `sys.*`, test read-only), `tools/kb-import/` (Drive/PDF/CHM/ZIP/DDL → Markdown, upload, promocja, build), `deploy/scripts/wg_guard.sh` bez wyjątków; runbook `docs/runbooks/new-kb-bulk-import.md`; pierwsza baza: `SubiektKB` |
| 2026-09-11 | Brak decyzji o dostępie agenta (Claude Code na hoście) do PRODUKCYJNEJ bazy Subiekta GT | **Read-only MCP `mssql` na hoście z deny-listą danych osobowych i logiem zapytań** (`tools/mssql-introspect/mcp-server.mjs`, bramka `src/mcp-readonly.mjs`); wyłącznie sesja Claude Code roota na VPS, bez drogi kontener→tunel, bez panelu i kluczy zdalnych. Dane wierszowe klientów NIE trafiają do baz wiedzy (decyzja 2026-09-10 utrzymana); wartość instancji trafi do osobnej bazy IloveKB jako słowniki z allow-listy, konwencje, agregaty z progiem k i lista dostawców-osób prawnych. Opis i kontrole: `docs/data-governance.md` §1.3 droga 3. Właściciel delegował decyzję operatorowi („sam zadecyduj"). |
| 2026-09-23 | Kopia off-site backupu: obiektowy storage S3 (Contabo) przez rclone | **Drugi host operatora (pomagier, biuro) przez WireGuard + rsync/SSH**: użytkownik write-only (`rrsync -wo -no-del -no-overwrite`), bloby zamrażane `chattr +i` i retencja po stronie odbiorcy, dead-man's switch w Kumie; S3 pozostaje opcją równoległą (`rclone://`) | koszt 0 i zero nowych usług; S3 nie zostało skonfigurowane od 2026-09-07; ograniczenie: wspólny operator i tunel (chroni przed utratą hosta, nie przed utratą operatora) | `deploy/scripts/backup.sh` (offsite_rsync), `deploy/offsite/`, `docs/runbooks/disaster-recovery.md` 0b |
| 2026-09-23 | Profil 24 GB: Neo4j heap 2G / pagecache 1G, limit kontenera 4g | **Heap 3G, limit 5g** (pagecache bez zmian) — przy trzech bazach (SubiektKB 35 tys. chunków + IloveKB + AnalizyERP) pełna przebudowa wpadała w burzę GC (stop-the-world 1–2 s, topic.csv >30 min); po zmianie 0 pauz | host ma 24 GB, po zmianie ~9 GB wolne; alternatywa (mniejsze partie builda) nie istnieje w builderze OpenSPG | `deploy/kag/.env` (NEO4J_HEAP/NEO4J_MEM_LIMIT), `docs/runbooks/typowe-awarie.md` §2 |
