...
- Indeks TextAndVector tylko na contentPreview (800 zn.) obniża recall względem indeksowania pełnego chunka — mitigacja: chunking wyrównany do nagłówków/akapitów (kluczowa treść na początku), summary dokumentu też indeksowane, retrieval dociąga pełny content po trafieniu preview; jeśli po testach QA recall za niski, faza 2: podniesienie previewLen lub obniżenie maxLen chunka (zmiana bezpieczna — UPSERT po deterministycznych id).
