...
- **mcp (kag-mcp):** własny obraz `node:22-alpine`, ten sam hardening; [NADPISANE przez PLAN.md: wspólny wolumen SQLite WAL; internal API zostaje tylko do cache-invalidate] czyta bazę kluczy przez wewnętrzne API panelu (`http://kag-panel:8080/internal/...` z tokenem serwisowym) — NIE współdzieli pliku SQLite (jeden pisarz). Port 3001 tylko w sieciach docker.
