...
- **Akronimy jako dokładne tokeny + rerank z zachowaniem partycji** (2026-09-14, profil trzech baz SubiektKB+IloveKB+AnalizyERP): akronim 3–8 liter z ≥2 wielkimi (`DSO`, `DIO`, `KSeF`, `JPK`) jest dokładnym tokenem jak wersja/identyfikator; rerank `embed` porządkuje cosinusem WEWNĄTRZ grup „komplet tokenów (`exact_match`)" i „reszta", nie między nimi — inaczej nagłówki dokumentów KPI (słowa kluczowe + opis) wypychały sekcję z właściwym szablonem poza okno 6 źródeł. Pełny eval 174 goldens: hit@1 0.854→0.89, MRR 0.905→0.932.
- **answer-v4 — pełna odmowa jest krótka, decyzja w kodzie** (2026-09-14): treść promptu = v3. (a) Odpowiedź zaczynająca się formułą niewiedzy („Nie wiem…", „Źródła nie zawierają…", `I don't know`), do 500 zn., jest `noAnswer` (luka `out_of_scope`, bez cache) także gdy model pominął `SCOPE:` — 3/4 sond near-miss („adres klienta Kowalskiego") szło jako odpowiedź z pewnością ~0.8. (b) `SCOPE:` przy odpowiedzi dłuższej niż 500 zn. jest ignorowany z ostrzeżeniem — model stawiał go na 5/56 realnych odpowiedzi (procedura KSeF, kolumny `nz__Finanse`, czerwony PLUS). Rozszerzenie reguły SCOPE w prompcie dało 22/40 fałszywych odmów na sondach SubiektKB — odrzucone; znacznik jest u modelu niestabilny (12–17 odmów w identycznych biegach 40 sond). Narzędzia: `isRefusalText`/`REFUSAL_MAX_CHARS`, harness `tools/eval/answer-offline.mjs --ns A,B,C` z polami sond `expectNs`/`expectRefusal` (kolumna `*` = SCOPE od modelu).
