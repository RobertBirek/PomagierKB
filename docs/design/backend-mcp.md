...
- **answer-v4 — odmowa bez znacznika SCOPE** (2026-09-14): treść promptu = v3; odpowiedź zaczynająca się formułą niewiedzy („Nie wiem…", „Źródła nie zawierają…", `I don't know`), krótsza niż 500 zn., jest `noAnswer` (luka `out_of_scope`, bez cache) także gdy model pominął `SCOPE:`. Powód: 3/4 sond near-miss („adres klienta Kowalskiego") szło jako odpowiedź z pewnością ~0.8. Rozszerzenie reguły SCOPE w prompcie dało 22/40 fałszywych odmów na sondach SubiektKB — odrzucone; sam znacznik jest u modelu niestabilny (12–17 odmów w identycznych biegach 40 sond), więc decyzja zapada w kodzie (`isRefusalText`, harness `tools/eval/answer-offline.mjs --ns A,B,C` z polami sond `expectNs`/`expectRefusal`).
