...
try {
  for (const p of probes) {
    const t0 = Date.now();
    const raw = await client.page.evaluate(async (q) => {
      const res = await fetch('/api/v1/ask', { method: 'POST', credentials: 'include', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ question: q }) });
      return { status: res.status, text: await res.text() };
    }, p.q);
    const m = /event: result\ndata: (.*)/.exec(raw.text);
    const d = m ? JSON.parse(m[1]) : null;
    const sources = [...new Set((d?.citations ?? []).map((c) => String(c.title).split(' — ')[0]))];
    const r = { q: p.q, kind: p.kind, ms: Date.now() - t0, status: raw.status, noAnswer: d?.noAnswer ?? null, confidence: d?.confidence ?? null, citations: (d?.citations ?? []).length, sources, answerId: d?.answerId ?? null, answer: (d?.answer ?? '').slice(0, 400) };
    results.push(r);
    console.log(`${String(r.ms).padStart(6)} ms | ${r.noAnswer ? 'ODMOWA' : 'odp.  '} | conf ${r.confidence ?? '-'} | cyt ${r.citations} | ${r.kind.padEnd(9)} | ${p.q.slice(0, 70)} → ${sources.join(', ')}`);
  }
} finally {
  await client.close();
}
