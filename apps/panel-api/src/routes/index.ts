...
export default async function registerRoutes(app: FastifyInstance): Promise<void> {
  ...
  await app.register(draftsRoutes, { prefix: '/api/v1' }); // routes/drafts.ts (Inbox)
  await app.register(contentRoutes, { prefix: '/api/v1' }); // routes/content.ts (intake pipeline)
  ...
  await app.register(learningRoutes, { prefix: '/api/v1' }); // routes/learning.ts (luki wiedzy)
